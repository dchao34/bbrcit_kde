...
class KernelDensity : public Kdtree<D,PAttrT,NAttrT,FloatT> {
  ...
  public: 
    ...
    KernelDensity();
    KernelDensity(const std::vector<DataPointType>&, double, int leaf_nmax=2);
    KernelDensity(std::vector<DataPointType>&&, double, int leaf_nmax=2);
    ...
    FloatT eval(const GeomPointType&, FloatType) const;
...
template<int D, typename KT, typename PT, typename NT, typename FT>
KernelDensity<D,KT,PT,NT,FT>::KernelDensity() : KdtreeType() {}

template<int D, typename KT, typename PT, typename NT, typename FT>
KernelDensity<D,KT,PT,NT,FT>::KernelDensity(
    const std::vector<DataPointType> &pts, double bw, int nmax) 
  : KdtreeType(pts, nmax), bandwidth_(bw) {}

template<int D, typename KT, typename PT, typename NT, typename FT>
KernelDensity<D,KT,PT,NT,FT>::KernelDensity(
    std::vector<DataPointType> &&pts, double bw, int nmax) 
  : KdtreeType(std::move(pts), nmax), bandwidth_(bw) {}
