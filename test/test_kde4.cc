...
//        equally weighted. 
// kernel: 2d epanechnikov product kernel.

int main() {

  using KernelDensityType = KernelDensity<2,EpanechnikovProductKernel<2>>;
  ...
  kde.kernel().set_bandwidth(0, 0.2);
  kde.kernel().set_bandwidth(1, 0.05);
  ...
  fout.open("test_kde4_single.csv");
  write_kde2d_result(fout, queries, 
                     start_x, end_x, steps_x,
                     start_y, end_y, steps_y);
  fout.close();
