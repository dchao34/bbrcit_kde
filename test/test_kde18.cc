...
int main() {
  ...
  cout << "+ least squares cross validation: " << endl;

  cout << endl;

  vector<pair<double, double>> bandwidths_pair;
  double start_x = 0.08, end_x = 0.25; int steps_x = 10;
  double start_y = 0.05, end_y = 0.15; int steps_y = 10;
  ...
  fout.open("test_kde18_cv.csv");
  write_cv2d_result(fout, cv_scores, 
  ...
  kde.kernel().set_bandwidths(0.16, 0.078);
  ...
  fout.open("test_kde18_adapt.csv");
  write_kde2d_result(fout, queries, 
                     start_x, end_x, steps_x,
                     start_y, end_y, steps_y);
  fout.close();
