...
int main() {
  ...
  elapsed = end - start;
  cout << "  run time: " << elapsed.count() << " ms. " << std::endl;

  fout.open("test_kde19_tree.csv");
  ...
  // 5. simulate from kde
  size_t n_sim_pts = 10000;
  cout << "+ simulating " << n_sim_pts << endl; 
  std::vector<DataPointType> sim_pts;

  start = std::chrono::high_resolution_clock::now();
  for (size_t i = 0; i < n_sim_pts; ++i) {
    sim_pts.push_back(kde.simulate(e));
  }
  end = std::chrono::high_resolution_clock::now();
  elapsed = end - start;
  cout << "  run time: " << elapsed.count() << " ms. " << std::endl;

  fout.open("test_kde19_sim.csv");
  write_scatter_data(fout, sim_pts);
  fout.close();
